...
impl AdaptiveLimitManager {
    ...
    pub fn get(&self, account_id: &str) -> Option<dashmap::mapref::one::Ref<'_, String, AdaptiveLimitTracker>> {
...
mod tests {
    ...
    fn test_persisted_with_decay() {
        let fresh = AdaptiveLimitTracker::from_persisted(
            100, 100, 0, 0.85, AIMDController::default()
        );
        assert_eq!(fresh.confirmed_limit(), 100);

        let stale = AdaptiveLimitTracker::from_persisted(
            100, 100, 86400 * 2, 0.85, AIMDController::default() // 2 days old
        );
